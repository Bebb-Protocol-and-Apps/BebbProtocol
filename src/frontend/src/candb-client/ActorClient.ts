...
> {
  ...
  async update<F extends (...args: any[]) => ReturnType<F>>(
    ...
    console.log("Debug ActorClient update pk ", pk);
    console.log("Debug ActorClient update sk ", sk);
    const canisterIds = await this.indexClient.getCanistersForPK(pk, false);
    console.log("Debug ActorClient update canisterIds ", canisterIds);
    ...
    console.log("Debug ActorClient update matchingSK ", matchingSK);
    ...
    const canisterToMakeUpdateCallTo =
      matchingSK.length === 0
        ...
        : // otherwise insert into the first (and only) matching canisterId
          matchingSK[0];
    console.log("Debug ActorClient update canisterToMakeUpdateCallTo ", canisterToMakeUpdateCallTo);
    const updateCanisterActor = createActor<ActorCanisterType>({
      ...this.actorOptions,
      canisterId: canisterToMakeUpdateCallTo,
    });
    console.log("Debug ActorClient update updateCanisterActor ", updateCanisterActor);
